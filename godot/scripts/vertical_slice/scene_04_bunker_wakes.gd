...
# PROTOCOL ZERO — Milestone 0.4 / O Bunker Desperta.
# Promessa única: o jogador sente a passagem de instalação morta para instalação viva.
# Greybox deliberado: sem CORE, sem camas, sem grupo e sem arte final.
...
func _add_static_box(name_: String, size: Vector3, pos: Vector3, color: Color, parent: Node = self) -> StaticBody3D:
...
func _build_lights() -> void:
	...
	distant_room_light = OmniLight3D.new()
	distant_room_light.name = "DistantRoomLight"
...
func _build_camera(target_player: CharacterBody3D) -> Camera3D:
	...
	camera.set("follow_min", Vector2(-0.70, -6.0))
	camera.set("follow_max", Vector2(0.70, 4.4))
...
func _build_ui() -> void:
	...
	objective_label.text = "CENA 4 — CZI-07"
...
func _on_generator_interact_pressed() -> void:
	...
	objective_label.text = "CENA 4 — O BUNKER DESPERTA"
	status_label.text = "ENERGIA AUXILIAR // PARTIDA..."
	_start_mobile_safe_drone()
	_run_power_sequence()

func _start_mobile_safe_drone() -> void:
	# Chamado pelo callback do botão: o próprio INTERAGIR fornece o gesto exigido pelo navegador mobile.
...
func apply_power_stage_for_test(stage: int) -> void:
	# Smoke test usa a mesma lógica de estado sem esperar cinco segundos de relógio real.
	power_started = true
