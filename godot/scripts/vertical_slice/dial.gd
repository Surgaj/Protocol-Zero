...
func setup(min_value: float, max_value: float, start_value: float) -> void:
	...
	current_frequency = clampf(start_value, min_frequency, max_frequency)
	locked = false
	lock_progress = 0.0
	_update_labels()
	queue_redraw()
...
func set_feedback(value: float, new_proximity: float, in_lock_zone: bool, new_lock_progress: float) -> void:
	...
	proximity = clampf(new_proximity, 0.0, 1.0)
	lock_progress = clampf(new_lock_progress, 0.0, 1.0)
	...
	if _progress_bar != null:
		_progress_bar.scale.x = maxf(0.001, lock_progress)
...
func _update_from_local_x(local_x: float) -> void:
	var track_left: float = 44.0
	var track_right: float = maxf(track_left + 1.0, size.x - 44.0)
	var ratio: float = clampf((local_x - track_left) / (track_right - track_left), 0.0, 1.0)
	current_frequency = lerpf(min_frequency, max_frequency, ratio)
...
func _draw() -> void:
	...
	var track_left: float = 44.0
	var track_right: float = maxf(track_left + 1.0, size.x - 44.0)
	var track_y: float = size.y * 0.58

	draw_line(Vector2(track_left, track_y), Vector2(track_right, track_y), Color(0.38, 0.38, 0.38), 5.0)

	for i: int in range(13):
		var t: float = float(i) / 12.0
		var x: float = lerpf(track_left, track_right, t)
		var h: float = 18.0 if i % 3 == 0 else 10.0
		draw_line(Vector2(x, track_y - h * 0.5), Vector2(x, track_y + h * 0.5), Color(0.62, 0.62, 0.62), 2.0)

	var value_ratio: float = inverse_lerp(min_frequency, max_frequency, current_frequency)
	var knob_x: float = lerpf(track_left, track_right, value_ratio)
	var glow_radius: float = 22.0 + proximity * 10.0
...
func _build_labels() -> void:
	...
	progress_bg.size = Vector2(maxf(1.0, size.x - 56.0), 8)
